...
".(function_exists("processTD")?processTD("<div class='genesis-item-property '>
	...
	function get_agregate()
	{
		...
		$sql = "SELECT 1 as stub  FROM (SELECT main_table.* , (select text FROM (SELECT name as text, id as value FROM products) tmp_e450efc5 WHERE value=main_table.product_id) as product_id_text FROM _orders_products_grouped main_table) temp $srch $filter $where $order";
	...
	function get_data($force_kill_pagination=false)
	{
		...
		if($pagination == 1)
		{
			$sql = "SELECT SQL_CALC_FOUND_ROWS * FROM (SELECT  main_table.* , (select text FROM (SELECT name as text, id as value FROM products) tmp_e450efc5 WHERE value=main_table.product_id) as product_id_text FROM _orders_products_grouped main_table) temp $srch $filter $where $order LIMIT :start, :limit";
		...
		{
			$sql = "SELECT SQL_CALC_FOUND_ROWS * FROM (SELECT main_table.* , (select text FROM (SELECT name as text, id as value FROM products) tmp_e450efc5 WHERE value=main_table.product_id) as product_id_text FROM _orders_products_grouped main_table) temp $srch $filter $where $order";
			if(in_array($_SERVER['SERVER_NAME'], ["test-genesis.alef.im", "devtest-genesis.alef.im", "localhost"]) || $debug)
			{
				echo "<!--SQL DATA {$sql} -->";
			}
